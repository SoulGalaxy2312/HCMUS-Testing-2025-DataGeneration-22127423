package data_generation.data_generation.DataGeneration.db;

import java.io.*;
import java.sql.Timestamp;
import java.util.*;

import org.springframework.core.io.ClassPathResource;
import org.springframework.stereotype.Component;


import data_generation.data_generation.DataGeneration.entity.User;

@Component
public class CsvUserReader {

    public List<User> readUsersFromCsv(String fileName) {
        List<User> users = new ArrayList<>();

        try (BufferedReader reader = new BufferedReader(
                new InputStreamReader(new ClassPathResource(fileName).getInputStream()))) {

            String[] header = reader.readLine().split(","); // skip header
            String line;

            while ((line = reader.readLine()) != null) {
                String[] values = line.split(",");
                User user = new User();
                user.setFirstName(values[0]);
                user.setLastName(values[1]);
                user.setAddress(values[2]);
                user.setCity(values[3]);
                user.setState(values[4]);
                user.setCountry(values[5]);
                user.setPostcode(values[6]);
                user.setPhone(values[7]);
                user.setDob(values[8]);
                user.setEmail(values[9]);
                user.setPassword(values[10]);
                user.setRole(values[11]);
                user.setEnabled("1".equals(values[12]));
                user.setFailedLoginAttempts(Integer.parseInt(values[13]));
                user.setCreatedAt(new Timestamp(System.currentTimeMillis()));
                user.setUpdatedAt(new Timestamp(System.currentTimeMillis()));

                users.add(user);
            }
        } catch (Exception e) {
            System.err.println("Error reading CSV: " + e.getMessage());
        }

        return users;
    }
}
